...
namespace ElaroApi.Controllers
{
    ...
    public class AdresController : ControllerBase
    {
        ...
        // GET: api/adres
        [HttpGet]
        public async Task<IActionResult> GetAllAddresses([FromQuery] int? musteriId)
        {
            if (musteriId == null)
                return BadRequest(new { message = "Adres listesi icin musteriId gereklidir." });

            var adresler = await _context.Adresler
                .Where(a => a.MusteriID == musteriId.Value)
                .ToListAsync();
