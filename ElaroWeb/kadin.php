...
$elbiseler = urunleriGetir($baglanti, 1);
$ayakkabilar = urunleriGetir($baglanti, 2);
$cantalar = urunleriGetir($baglanti, 3);
$aksesuarlar = urunleriGetir($baglanti, 4);
?>
...
<!-- Kadın Çantaları -->
<section class="urunler-bolumu">
  <h2>Kadın Çantaları</h2>
  <div class="urunler-grid">
    <?php foreach ($cantalar as $urun): ?>
    <div class="urun-karti">
      <img src="<?= htmlspecialchars($urun['GorselURL']) ?>" alt="<?= htmlspecialchars($urun['Adi']) ?>">
      <h4><?= htmlspecialchars($urun['Adi']) ?></h4>
      <p><?= number_format($urun['Fiyat'], 2) ?> TL</p>
